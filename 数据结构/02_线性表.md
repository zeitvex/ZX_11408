...
> [!IMPORTANT] 考点追踪：顺序表上操作的时间复杂度分析（高频：2023）与顺序表的应用（真题热点：2010、2011、2018、2020、2025）
...
> [!IMPORTANT] 核心结论：时间复杂度层级速记 (2025 必备)
> 1. **常数阶 $O(1)$**: 与规模无关。
> 2. **对数阶 $O(\log n)$**: 常见于折半查找、二叉搜索树、分治法的一侧。
> 3. **线性阶 $O(n)$**: 遍历数组、单链表扫描。
> 4. **线性对数阶 $O(n \log n)$**: 快速排序、归并排序、堆排序的平均情况。
> 5. **平方阶 $O(n^2)$**: 嵌套循环（冒泡、直接插入排序）。
> $O(1)<O(\log_2n)<O(n)<O(n\log_2n)<O(n^2)<O(n^3)<O(2^n)<O(n!)<O(n^n)$

> [!IMPORTANT] 考点追踪：分析算法的时间复杂度（高频：2010—2021、2025）
> 1. **插入/删除**：最好 O(1)，最坏 O(n)，平均 O(n)。空间复杂度 O(1)（原地工作）。
> 2. **按序查找（GetElem）**：直接通过下标访问，O(1)。这是顺序表唯一的随机存取特性。
> 3. **按值查找（LocateElem）**：无序时 O(n)；若表有序，可降至 $O(\log n)$（折半查找）。

> [!WARNING] 易错点：位序与下标（高频）
...
> [!IMPORTANT] 考点追踪：单链表操作技巧（2016、2025）
> 1. **头插法（逆序）**：常用于链表逆置、多项式相加。
> 2. **尾插法（正序）**：必须维护一个尾指针 `r`。
> 3. **前插操作**：若只给出 *p 指针，可通过交换 *p 与 *s 的 `data` 变相实现（O(1)）。
> 4. **删除 p 结点**：若只给出 *p 指针，可通过复制后继结点 `data` 后删除后继变相实现（O(1)），注意 p 是尾结点时无法使用此法。

> [!WARNING] 注意：头结点与判空
> 1. **带头结点**：判空为 `L->next == NULL`。
> 2. **不带头结点**：判空为 `L == NULL`。
> 在算法设计中，统考真题默认多为**带头结点**，除非显式说明。

> [!IMPORTANT] 核心结论：单链表常用操作复杂度
> - **头插法/尾插法建立**：$O(n)$。
> - **按序号查找/按值查找**：$O(n)$。
...
> [!IMPORTANT] 核心结论：循环单链表 (2025 考点)
> 1. **判空**：`L->next == L`。
> 2. **特长**：若设有尾指针 `rear`，访首元节点 (`rear->next->next`) 和访尾节点 (`rear`) 均为 $O(1)$。
