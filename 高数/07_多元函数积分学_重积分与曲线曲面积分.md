...
设 $L $ 是 xOy 面内从点 $ A$ 到点 $B $ 的一条有向光滑曲线弧，函数 P(x,y),Q(x,y) 在 $ L$ 上有界在 $L$
上插入点 $M_1(x_1,y_1),M_2(x_2,y_2),\dots,M_{n-1}(x_{n-1},y_{n-1})$，把 $L $ 分为 $ n$ 个有向弧段
$\stackrel\frown{M_{i-1}M_i}(i=1,2,\dots,n;M_0=A,M_n=B)$ 设
...
当参数 $t$ 单调地从 $\alpha$ 变到 $\beta $ 时，点 M(x,y) 从 $ L$ 的起点 $A $ 沿着 $ L$ 运动到终点
$B$，则：
...
**格林公式**如下：设闭区域 $D $ 由分段光滑的曲线 $ L$
（取正向的边界曲线）围成，如果函数 P(x,y) 和 Q(x,y) 在 $D$ 上具有一阶连续偏导数，则有：
...
设区域 $G $ 但是一个单连通区域如果函数 P(x,y) 与 Q(x,y) 在 $ G$ 内具有**一阶连续偏导数** 则曲线积分
$\int_LP\mathrm dx+Q\mathrm dy $ 在 $ G$ 内路径无关的充要条件为：
...
与每个小曲面上点的取法无关则称极限 $\lim_{\lambda\to0}\sum_{i=0}^nf(\xi_i,\eta_i,\zeta_i)\Delta S_i$
为 **函数 f(x,y,z) 在曲面 $\Sigma$ 上<u>对面积的曲面积分</u>或 <u>第一类曲面积分</u>** 记作：
