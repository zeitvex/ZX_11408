...
$E $ 的边界点的全体，称作 $ E$ 的**边界**，记为 $\part E $（边界点未必是点集 $ E$
内的点，也就是 < 和 ≤ 的区别）
...
利用克拉默法则即可轻易求得 $\frac{\part u}{\part x},\frac{\part v}{\part x}$，而
**Jacobi 行列式正是这里的系数矩阵**
...
>
> 求表面积为 $a^2$
> 而体积最大的长方体的体积。长方体的体积：$V=xyz$，但是由于问题中的条件，三个自变量还具有制约关系：$
> 2(xy+yz+xz)=a^2$
...
$A=\frac{\part z}{\part x},B=\frac{\part z}{\part y}$。但是，有时候无法直接对 f
(x,y) 求偏导，但这并不意味着凑不出
$\Delta z=A\Delta x+B\Delta y+o (\sqrt{(\Delta x)^2+(\Delta y)^2})$
