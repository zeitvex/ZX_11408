...
#### 2. 技巧：表格法 (快速分部积分)
适用于 $\int P_n(x) \cdot f(x) \mathrm{d}x$ 类型，其中 $P_n(x)$ 为多项式，f(x) 为 $e^{ax}, \sin ax, \cos ax$ 等。
**步骤**：
...
2. **重因式**：若分母含有 $(x-a)^k$，产生 $k$ 项 $\frac{A_1}{x-a} + \frac{A_2}{(x-a)^2} + \dots + \frac{A_k}{(x-a)^k}$。
...
*   **抽水做功**：将容器中的水抽出。取深度 $x$ 处的水平截面面积为 A(x)，将厚度为 $\mathrm{d}x$ 的水层提升到顶部（路程为 $x$）所需的功元为 $\mathrm{d}W = \rho g A(x) x \mathrm{d}x$。
    $$W = \int_0^h \rho g A(x) x \mathrm{d}x$$

### 2. 水压力
垂直浸没在液体中的平板（一侧）受到的压力。取深度 $x$ 处的宽度为 l(x)，则压力元为 $\mathrm{d}P = \rho g x l(x) \mathrm{d}x$。
...
*注意：这与定积分中值定理 $f(\xi) = \frac{1}{b-a} \int_a^b f(x) \mathrm{d}x$ 紧密相关。*
...
- 二重积分换极坐标时，被积函数后须乘 $r$：$\iint f\,dxdy = \iint f(r\cos\theta, r\sin\theta)\cdot r\,dr\,d\theta$
...
- 连续的奇函数，其所有原函数都是偶函数。
- 连续的偶函数的原函数中，**有且仅有一个**是奇函数，即 $\int_0^x f(t)\,dt$；加任意非零常数 $C$ 就破坏奇函数条件。
