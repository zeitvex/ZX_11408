...
- 伯努利（Bernoulli）大数定律
- 辛钦（Khinchin）大数定律
- 棣莫弗-拉普拉斯（DeMoivre-Laplace）定理
- 列维-林德伯格（Lévy-Lindeberg）定理

## 本章脉络

| 定理 | 由何而来 | 适用前提 |
| :--- | :--- | :--- |
| 切比雪夫不等式 | 马尔可夫不等式 | 期望、方差存在 |
| 切比雪夫大数定律 | 由切比雪夫不等式**直接推导** | 相互独立、方差有共同上界 |
| 辛钦大数定律（弱大数定律） | 切比雪夫大数定律在**独立同分布**下的推广 | 独立同分布、期望存在 |
| 伯努利大数定律 | 辛钦大数定律**应用于伯努利分布** | $n$ 重伯努利试验 |
| 列维-林德伯格定理（中心极限定理） | —— | 独立同分布、期望与方差存在 |
| 棣莫弗-拉普拉斯定理 | 中心极限定理**应用于二项分布** | $X\sim B(n,p)$，$n$ 充分大 |

一句话记忆：**大数定律讲「均值收敛到期望」，中心极限定理讲「和的分布趋于正态」**。
两条支线各有一个「伯努利／二项分布特例」。
...
>
> 发现 $X\ge aI_{X\ge a}$，这是因为，当 $X\ge a$ 时，$I_{X\ge a}=1$，自然也满足
> $X\ge aI_{X\ge a}$；而当 $X<a$ 时，$I_{X\ge a}=0$，而 $X>0$，因此该不等式也满足
>
...
> 以此为基础推导切比雪夫不等式：
>
> 对于任意随机变量 $X$，它具有数学期望 $\mu$，方差 $\sigma^2$，那么，对于
> $\forall \varepsilon>0$，根据马尔可夫不等式，直接有：
>
> $$
...
_切比雪夫（Chebyshev）大数定理，也称切比雪夫不等式，是概率论中的一个重要结果，用于估计随机变量的分布偏离其期望值的概率。定理表明，对于任何具有有限期望值和有限方差的随机变量，其值偏离期望值超过某个特定范围的概率是有界的。_
...
_切比雪夫不等式的重要性在于它在没有关于随机变量的分布形状的任何假设下，提供了一个普遍适用的界限。这使得它在许多领域中都具有广泛的应用，包括**统计学**、工程学和金融学等。_
...
设 $X_1,X_2,\cdots$ 是相互独立且服从同一分布的随机变量序列，且具有数学期望
$E(X_k)=\mu,k=1,2,\cdots$，则有：
...
\right|<\varepsilon
\right\}=1\\
$\forall\varepsilon>0, \lim_{n\to\infty} P\left\{ \left| \frac1n\sum_{k=1}^nX_k-\mu \right| < \varepsilon \right\} = 1
\\
\forall\varepsilon>0, \lim_{n\to\infty} P\left\{ \left| \frac1n\sum_{k=1}^nX_k-\mu \right| \ge \varepsilon \right\} = 0
$$

也就是说，对于相互独立且服从同一分布，且数学期望为 $\mu$ 的随机变量 $X_1,\cdots,X_n$。当 $n$
很大时，它们的算术平均 $\cfrac1n\sum_{k=1}^n X_k$ 大概率接近 $\mu$
...
设随机变量 $X_1,X_2,\cdots,X_n,\cdots$ 相互独立且服从同一分布且具有数学期望和方差
$E(X_k)=\mu,D(X_k)=\sigma^2,k=1,2,\cdots$ 则随机变量之和 $\sum_{k=1}^n X_k$ 的标准化变量
...
设随机变量 $X_1,X_2,\cdots,X_n,\cdots$ 相互独立，且分别具有数学期望和方差
$E(X_k)=\mu_k,D(X_k)=\sigma_k^2,k=1,2,\cdots$ 记 $B_n^2=\sum_{k=1}^n\sigma_k^2$

如果

$$
\exists\delta>0,\lim_{n\to\infty}\frac1{B^{2+\delta}_n}\sum_{k=1}^nE\{|X_k-\mu_k|^{2+\delta}\}=0
$$
